...
---
To motivate the need for an alternative to neural networks (and more generally, gradient-based learning), this post will describe the major problems that in my view prevent neural networks from reaching human-level intelligence. The future posts in this series will document my attempt at creating a Minecraft AI that surpasses the ability of current neural networks using a completely different approach.

### **Problems with Neural Networks**
### Continual Learning
Arguably the biggest issue with neural networks as an approach to artifical intelligence is that they are unable to learn from new data once they are "trained". Training a neural network requires curating a training set that you hope captures enough information about the given task to perform well on a test set, which is meant to accurately reflect the data you expect your model to see when it's deployed. There's no way around this split between training samples and testing samples because neural networks are nothing but continous function approximators from discrete samples.
...
I think we should be designing perception algorithms that are loosely inspired by observations about human perception, incorporating attention and predictive coding, and taking advantage of built-in priors like lines and curves. We should also be actively seeking a learnable representation scheme that uses abstracton, disentanglement and compositionality to quickly generalize to new tasks. Although this is a very vague description of what I think AI will require, it's clear that neural networks do not fit the description, so we must look for something new to satisfy those requirements.

In the next post I'll cover an outline of what I think such an algorithm will look like, as well as some missing pieces that I don't quite know how to implement yet.